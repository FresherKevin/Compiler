...
public class YufaCompiler {
	...
	private void Compiler(){
        ...
        {
        	Instruct instruct=Code.elementAt(i);
	...
	{			
		...
		{// current 存放单词的种别码
			 ...
			 * 赋值语句,第一个单词为变量标识符
			 */
			else if(current.equals(SYM._ID))
			{
			 ...
			 * 当循环语句
			 */
			else if(current.equals(SYM._while))
			{
				//System.out.println("while sentence "+level+" "+current);
				int hereInwhile=index;//记录while循环开始的地方,无条件跳转到的地方
				Condition(level,currenttx);//条件
		...
		if(current!=null&&current.equals(SYM._LEFT))//左括号
		{
			if((current=getNextSYM())!=null&&current.equals(SYM._ID))//标示符
			{
				...
				if(inagr==0)
				{
					...
					GEN(FunctionCode.STO,currentItem.level,currentItem.address);
				}
				else
				{
					//GEN(FunctionCode.STO,currentItem.level,currentItem.address);
					GEN(FunctionCode.OPR,currentItem.address,OPR.WRITE);//write操作
				}
