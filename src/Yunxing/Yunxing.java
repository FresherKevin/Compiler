	...
	*/
	...
	private void OneInstruct(){
		...
		case STO:
			int addr=0;
			if (ireg.abslevel==0) {
				addr=ireg.addr+next_size;
			}
			else {
				addr=ireg.addr;
			}
			tmp=stack[top];//栈顶内容存放到变量中
			stack[addr]=tmp;//存到变量地址中
			stack[top--]=0;
			printStack();
			break;
		case CAL:
		...
		case OPR:// 关系和算术运算
			...
			case OPR.LT://关系小于操作
				...
				break;
			case OPR.WRITE://写操作
				//Instruct ireg_before=Code.elementAt(pc--);
				//writeMem(ireg_before.addr);//将参数的内容打印出来
				
				writeMem(ireg.abslevel);//将参数的内容打印出来
				printStack();
				break;
			case OPR.READ://读操作
